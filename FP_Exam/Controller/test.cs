...
namespace FP_Exam.Controller
{
	public class test : LoginController
	{
		...
		protected string FmAnswer(string content, int qid, int en)
		{
			...
			for (int i = 0; i < array2.Length; i++)
			{
				...
				if (flag)
				{
					content = content + str + string.Format("<input type=\"text\" id=\"answer_1_{0}\" name=\"answer_{1}\" value=\"\" class=\"tkt\"/>", en, qid);
				}
				else
				{
					content += str;
