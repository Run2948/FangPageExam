<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="FP_Course.Controller.course_view" %>
...
protected override void View()
{
	...
	ViewBuilder.Append("<title>" + echo(courseinfo.name) + "|在线课程|" + echo(pagetitle) + "</title>\r\n");
	...
	ViewBuilder.Append("            <li class=\"active\">" + echo(courseinfo.name) + "</li>\r\n");
	ViewBuilder.Append("          </ul>\r\n");
	ViewBuilder.Append("        </div>\r\n");
	...
	ViewBuilder.Append("                  <div class=\"name\" style=\"font-weight:bold;\">" + echo(item.name) + "</div>\r\n");

	loop__id=0;
	foreach(VideoInfo video in GetVideoList(id,item.id))
	{
	loop__id++;
	ViewBuilder.Append("                  <div class=\"meta\" style=\"line-height:30px;\"><a href=\"course_video.aspx?videoid=" + echo(video.id) + "\" target=\"_blank\"><img src=\"" + echo(video.icon) + "\" style=\"width:16px;height:16px;\">" + echo(video.name) + "</a></div>\r\n");
	}//end loop
	ViewBuilder.Append("                </div>\r\n");
	}//end loop
	ViewBuilder.Append("              </div>\r\n");
